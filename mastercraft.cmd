...
if "%discipline" = "carving" then
{
	...
	if "%order.pref" = "wood" then var work.tools drawknife|rasp|shaper|carving.knife|clamp|stain|glue
}
...
action (order) var stain.order $1 when (\d+)\)\..*wood stain.*(Lirums|Kronars|Dokoras)
action (order) var glue.order $1 when (\d+)\)\..*wood glue.*(Lirums|Kronars|Dokoras)
...
action (book) var assemble2 $2 $3; var asmCount2 $1 when .*(\d).* finished bow (string)$
action (book) var assemble2 $2 $3; var asmCount2 $1 when .*(\d).* finished leather (strips)$
...
identify.order:
	...
	{
		matchre chapter.1 This logbook is tracking a work order requiring you to craft (forest information|wood pole|wood preparation|wood weapon repair|lumberjacking basics|bowcrafting instructions) from any material\.
		matchre chapter.2 This logbook is tracking a work order requiring you to craft (a toy bow|a farmer's bow|a practice shortbow|a shortbow|a simple shortbow|a sturdy shortbow|a light shortbow) from any material\.
		matchre chapter.3 This logbook is tracking a work order requiring you to craft (a simple longbow) from any material\.
		matchre chapter.4 This logbook is tracking a work order requiring you to craft (a stone band|a stone talisman case|a stone toe ring|a stone belt buckle|a stone nose ring|a stone armband|a stone pin|an articulated stone bracelet|a stone anklet|a stone tiara|a stone bracelet|a stone locket|a stone tailband|a stone choker|a stone hairpin|a stone diadem|a stone cloak pin|a stone circlet|a pair of stone earrings|an articulated stone necklace|a stone medallion|a stone crown|a stone amulet|an articulated stone belt|a stone pendant|a stone mask|a stone brooch|a stone earcuff) from any material\.
		matchre chapter.5 This logbook is tracking a work order requiring you to craft (arrowhead shaping|boar-tusk arrows|arrow shaft shaping|cougar-claw arrows) from any material\.
		matchre chapter.6 This logbook is tracking a work order requiring you to craft (bow lamination|bow lightening) from any material\.
		matchre chapter.7 This logbook is tracking a work order requiring you to craft (a wood band|a wood hairpin|a wood nose ring|a wood tailband|a wood toe ring|a wood pendant|a wood bracelet|a wood amulet|a wood anklet|a wood medallion|a wood pin|a pair of wood earrings|a wood cloak pin|a shallow wood cup) from any material\.
		matchre chapter.8 This logbook is tracking a work order requiring you to craft (a wood bead|a detailed wood totem|a detailed wood bead|a wood totem) from any material\.
		matchre chapter.9 This logbook is tracking a work order requiring you to craft (a wood cane|a wood walking cane|a wood nightstick|a wood quarterstaff) from any material\.
		matchre chapter.10 This logbook is tracking a work order requiring you to craft (a low wood table|a low wood table ) from any material\.
		matchre new.order.wait ^This logbook has been used to complete
		put read my %society.type logbook
		matchwait
	}
...
calc.material:
	...
	if (("%discipline" = "shaping") && ("%order.pref" = "wood")) then
	{
		pause .1
		action (book) on
		pause .5
		if !matchre("book", "$righthand") then send get my shaping book
		send read my book
		waitforre .*refined wood lumber .*\((\d+) (piece|pieces)\)
		var volume $1
		action (book) off
		pause 0.1
		var mass.volume %volume
		math mass.volume multiply %order.quantity
		gosub parts.inv
		if %mat.item.count > 0 then gosub count.material lumber
		if %mat.deed.count > 0 then gosub count.material deed
		pause 0.5
		echo Total Pieces Req'd: %mass.volume
		echo Pieces per Item: %volume
...
calc.parts:
	...
	if "%assemble2" = "string" then math asmCount2 subtract %string.count
	pause 2
...
parts.inv:
	...
	action (partsinv) math mat.item.count add 1 when ^\s+(a|an|some) (%work.material).*(leather|cloth|yarn|ingot|stack|lumber)$
	action (partsinv) math mat.deed.count add 1 when ^\s+a deed for (a|an|some).*(%work.material).*(leather|cloth|yarn|ingot|stack|lumber)$
	...
	action (partsinv) math stain.count add 1 when some wood stain
	action (partsinv) math glue.count add 1 when some wood glue
...
count.material:
	...
	action (count) math material.volume add $1 when You count out (\d+) (piece|pieces|yards) of (material|lumber) (remaining|there)\.$
	...
	if "%count" = "lumber" then
	{
		...
		gosub combine.check $MC_ENGINEERING.STORAGE "%work.material lumber"
	}
	if "%count" = "ingot" then
	{
...
process.order:
	...
	if "%discipline" = "shaping" then
	{
		...
		if "$righthand" != "Empty" then gosub verb put my $righthandnoun in my %main.storage
		pause 2
		if "%order.pref" = "wood" then
		{
			gosub gather.material lumber
			send count my lumber
			waitforre You count out (\d+) (piece|pieces) of lumber remaining
			if %volume > $1 then gosub small.mat lumber
			gosub verb get my %discipline book
			send study my book
			if $MC_DIFFICULTY < 3 then
				{
				put put %work.material lumber in my %main.storage
				goto new.order
				}
			pause .5
			gosub verb put my book in my %main.storage
...
lack.material:
	...
	if "%discipline" = "shaping" then
	{
		if "%order.pref" = "wood" then
		{
			...
			var order.type lumber			
		}
		if !contains("balsa|maple|pine", "%work.material") then goto lack.material.exit
...
first.order:
    if (("%discipline" = "carving")||("%discipline" = "tailor")||("%discipline" = "shaping"))  then
        {
