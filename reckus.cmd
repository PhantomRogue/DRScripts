...
###				stuff, and override normal    ###
###       options, just type:           ###
###					.reckus quiet               ###
###                                     ###
###########################################

### If you want to fold set these variables
	var folding no

# This is the name of the pattern (i.e. dolphin)
	var origami $origami

# This is where the pattern is (i.e. cloak, or primer in my cloak)
	var originstloc $originstloc

# This ensures we only fold once to success instead of to ML
	var singlefold yes

### If you want to recall set these variables
	var immortal no
	var herb no
	var famous no
	var holiday no

### If you want to perceive between instruments set this to yes
	var percing yes
...
### If you want to juggle while humming set these variables
	var juggling no

# The thing you want to juggle
	var jugglie $jugglie

# Where you keep that thing.
	var jugcon $jugcon
	var jugcounting yes
...
### Set instruments here, set to "no" if you don't want it used
	...
	var percu-instrument bones
	var string-instrument lyre
	var wind-instrument recorder
	var first strings
	var second winds
	var third percs
#	var third none
...
windtest:
	...
	pause
	if "%folding" = "yes" && $Mechanical_Lore.LearningRate < 34 then gosub origami.start
	pause
	if "%juggling" = "yes" && $Perception.LearningRate < 34 then gosub jug.start
	pause
	if "%cam" = "yes" && $Magical_Devices.LearningRate < 34 then gosub cam.start
	pause
	if "%percing" = "yes" && $Power_Perceive.LearningRate < 34 then gosub percit
	pause
...
#	put exp
#	waitfor EXP HELP
	if $%xskill.LearningRate > 26 then
	{
...
### Gosubs follow
playvocals:
	action return when eval $%xskill.LearningRate > 32
...
#	gosub earlycheck
	...
	if $Magical_Devices.LearningRate < 34 then gosub cam.start
	else pause 10
	if "%doneplay" = "yes" then return
#	gosub earlycheck
	if $Vocals.LearningRate > 32 then return
...
playinstrument:
	...
	if "%immortal" = "yes" && $Scholarship.LearningRate < 34 then
	{
		...
		waitforre Your memory seems a bit too clouded|You recall
	}
	else pause 14
	pause
#	gosub earlycheck
