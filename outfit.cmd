...
#debug 10
var BooksShort bl|we|arm|car|tail
var BooksLong blacksmith|weapon|armor|carving|tailoring
TOP:
   ...
   {
      if matchre("%BooksLong", "\b(%1)\b") then var BookType $1
      else exit
      shift
   }
   else goto USAGE
   if tolower("%1") = "build" then
   {
      var Item build
      goto BUILD
   }
...
BOOK_READ:
   action var ParsePageItem $1 when Page \d+: Instructions for crafting ((\S+(\s)?)+)    
   send read my %BookType book
   pause 1
   action remove Page \d+: Instructions for crafting ((\S+(\s)?)+)    =-
   eval ParsePageItem replacere("%ParsePageItem", "(an?|some|metal) ", "")
